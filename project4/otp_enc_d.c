 ...
 * Program: otp_enc_d																                    *
 * Description:  Program will run as daemon listening for connections.  It will receive a plaintext and key, encrypt, then send back the ciphertext.                *
...
void* threadRoutine(void *arg){
	...
	regex_t regex, otp_enc;	//Variable to hold regex
	regcomp(&regex, "@@", 0);	//'@@' is used to signa the end of the received message
	regcomp(&otp_enc, "otp_enc@@", 0); //this will be used to verify that the client sending messages is otp_enc
	while(1){
		...
		switch(isClientValid(socketFD)){
			...
			case 1: //Valid Client
				...
				plaintext = (char*)malloc(sizeof(char) * strlen(message));
				...
				sendCiphertext(ciphertext, socketFD);
				free(message);
				free(key);
				free(plaintext);
				free(ciphertext);
...
int isClientValid(int socketFD){
	...
	regcomp(&regex, "otp_enc@@", 0);	//regex for the first message that otp_enc will send
	char chunk[CHUNK_SIZE];
	memset(chunk, '\0', CHUNK_SIZE);
	int charsRead, retVal;
	char *vrfyMessage = "otp_enc_d@@";	//Verification message that server will send to otp_enc
